...
export class Recipe implements Entity {
    ...
    static getScavRecipes (): Recipe[] {
        ...
        for (const lootTable of scav.outputRefConditional) {
            ...
            const name = outputs.id.split('_');
            const rec = new Recipe(`${outputs.id}`, `${scav.name} Tier${name.pop()}`, scav.image, recipeInputs, outputs.data, scav.durationSeconds, scav.description, scav.cooldownSeconds, scav.category, scav.activeWhen, scav.outputRef.lootTableId, scav.outputRefConditional, scav.recipients)
            recipes.push(rec);
        }
        return recipes;
    }
