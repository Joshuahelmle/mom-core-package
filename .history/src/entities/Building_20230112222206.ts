...
export class Building implements IEntity {
    ...
    public static fromGameData ({ id, name, image, itemId, level, size, rarity, inventory, recipeRefs, keywords }: GameDataBuilding) {
        ...
        return new Building(id, name, image, inv, itemId, level, thisRarity, recipeRefs.length > 0 ? recipeRefs.map(rr => RECIPES.filter(r => r.category === rr.id)).flat() : [], size, recipeRefs[recLocation]?.concurrent ?? 0, upgrades, keywords, recipeRefs)
    }
}
